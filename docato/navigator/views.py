...
from navigator.utils import iterate_over_slot_values, iterate_over_frametypes, \
...
class Projects(LoginRequiredMixin, BaseCreateView, tables.djtab2.SingleTableView):
    template_name = 'navigator/projects.html'
    model = Project
    ...
    def form_valid(self, form):
        ...
        assign_perm('navigator.change_project', self.request.user, self.object)
        assign_perm('navigator.delete_project', self.request.user, self.object)
        return result
...
class ProjectPage(LoginRequiredMixin, BaseCreateView, tables.djtab2.SingleTableView):
    template_name = 'navigator/project.html'
    model = Subject
    ...
    def form_valid(self, form):
        ...
        assign_perm('navigator.change_subject', self.request.user, self.object)
        assign_perm('navigator.delete_subject', self.request.user, self.object)
        assign_perm('navigator.can_add_docs', self.request.user, self.object)
        assign_perm('navigator.can_edit_docs', self.request.user, self.object)
        assign_perm('navigator.can_edit_typesystem', self.request.user, self.object)
        return result
...
class SubjectPage(LoginRequiredMixin, SubjectMixin, BaseFormView, tables.djtab2.SingleTableView):
    template_name = 'navigator/subject.html'
    model = Document
...
class TypeSystem(LoginRequiredMixin, SubjectMixin, TemplateView):
    template_name = 'navigator/typesystem/typesystem.html'


class NewFrameType(LoginRequiredMixin, SubjectMixin, TemplateView):
    template_name = 'navigator/typesystem/frame_type.html'
...
class Doc(LoginRequiredMixin, SubjectMixin, DocMixin, TemplateView):
    template_name = 'navigator/doc.html'
    model = BaseSlotValue
...
def get_value_variants(request, **kwargs):
    ...
    if isinstance(sval, ClassLabelSlotValue):
        ...
        return render_to_response('navigator/typesystem/slot_values/select.html',
                                  {
                                   'value' : sval.value,
        ...
        return render_to_response('navigator/typesystem/slot_values/simple.html',
                                  {
...
class SearchFrameset(LoginRequiredMixin, TemplateView):
    template_name = 'navigator/search_frameset.html'


class SearchToolbar(LoginRequiredMixin, SubjectMixin, CreateView):
    template_name = 'navigator/search_toolbar.html'
    model = Document
    form_class = forms.AddDocumentForm
...
class SearchEmpty(LoginRequiredMixin, TemplateView):
    template_name = 'navigator/search_empty.html'
...
def do_search(request, eng_id, query):
    ...
    return render_to_response('navigator/engines/%s' % engine.template_name, args)
...
class Summary(LoginRequiredMixin, SubjectMixin, TemplateView):
    template_name = 'navigator/summary.html'
...
class Analysis(LoginRequiredMixin, SubjectMixin, TemplateView):
    template_name = 'navigator/analysis.html'
